...
def dss_config_files(dss_name, current_dir):
    os.chdir(f"{current_dir}/dss_batteries/")
    exp_dat_files = open('cim_test.dss', 'w')
    print(f'redirect {current_dir}/dss_batteries/{dss_name}.dss', file=exp_dat_files)
...
def upload_to_blazegraph(dss_name,fd_mrid, current_dir):
    ...
    print(f"\n\n----> uploading xml <----\n\n")
    print("-------------------------------------------------")
    os.system(f'curl -D- -H "Content-Type: application/xml" --upload-file {current_dir}/dss_batteries/{dss_name}.xml -X POST {CIMHubConfig.blazegraph_url}')
